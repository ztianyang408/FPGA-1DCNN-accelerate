...
DEFAULT_RESULTS_ROOT = REPO_ROOT / "phase2" / "results"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train an FPGA-friendly 1D CNN on FFT features")
    ...
    run_dir = args.results_root / f"{timestamp}_fpga_cnn"
    ...
    model = FPGAFriendlyRPMCNN().to(device)
    ...
    print(f"Training FPGA-friendly CNN on {device}...", flush=True)
    ...
    summary = {
        ...
        "fpga_design_notes": {
            "activations": "ReLU",
            "pooling": "MaxPool1d + global mean",
            "bn": False,
            "dropout": False,
            "target": "rpm regression",
        },
        "metrics": metrics,
    }
